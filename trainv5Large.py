...
def train_anti_overfitting():
    ...
    LEARNING_RATE = 1.5e-5  # Reduzido ainda mais
    ...
    PATIENCE = 5  # Ajustado
    LABEL_SMOOTHING = 0.15  # Reduzido drasticamente
    DROPOUT = 0.15  # Reduzido para início
    AUGMENT_PROB = 0.6  # Reduzido para início mais estável
    GRADIENT_CLIP_VAL = 0.5  # Reduzido para mais controle
    WARMUP_EPOCHS = 3  # Aumentado para início ainda mais gradual
    VALIDATION_BATCHES = 500  # Validar apenas parte do dataset para economizar tempo
    
    # Configurações do Cosine Annealing
    USE_COSINE_ANNEALING = True  # Ativar Cosine Annealing
    COSINE_T_MAX = 10  # Período do ciclo (reinicia a cada 10 épocas)
    USE_WARM_RESTARTS = True  # Usar Warm Restarts
    T_0 = 3  # Primeiro ciclo de 5 épocas
    T_MULT = 2  # Próximos ciclos: 10, 20, 40...
    ...
    logger.info(f"Scheduler: {'Cosine Annealing' + (' with Warm Restarts' if USE_WARM_RESTARTS else '') if USE_COSINE_ANNEALING else 'Linear'}")
    ...
    # Scheduler - Escolher entre Cosine Annealing ou Linear com Warmup
    if USE_COSINE_ANNEALING:
        if USE_WARM_RESTARTS:
            # Cosine Annealing com Warm Restarts
            scheduler = CosineAnnealingWarmRestarts(
                optimizer,
                T_0=T_0,  # Primeiro ciclo
                T_mult=T_MULT,  # Multiplicador para próximos ciclos
                eta_min=MIN_LR
            )
            logger.info(f"Using CosineAnnealingWarmRestarts - T_0: {T_0}, T_mult: {T_MULT}")
        else:
            # Cosine Annealing simples
            scheduler = CosineAnnealingLR(
                optimizer,
                T_max=COSINE_T_MAX,
                eta_min=MIN_LR
            )
            logger.info(f"Using CosineAnnealingLR - T_max: {COSINE_T_MAX}")
    else:
        # Scheduler original com warmup linear
    ...
    for epoch in epoch_pbar:
        ...
        for batch_idx, batch in enumerate(train_pbar):
            ...
            if (batch_idx + 1) % ACCUMULATION_STEPS == 0:
                ...
                # Scheduler step - diferente para cada tipo
                if USE_COSINE_ANNEALING and USE_WARM_RESTARTS:
                    # Para warm restarts, step a cada batch
                    scheduler.step(epoch + batch_idx / len(train_loader))
                elif not USE_COSINE_ANNEALING:
                    # Para scheduler linear, step a cada batch
                    scheduler.step()
        ...
        # Para CosineAnnealingLR simples, step após cada época
        if USE_COSINE_ANNEALING and not USE_WARM_RESTARTS:
            scheduler.step()
    ...
    if len(train_losses) > 0 and len(val_losses) > 0:
        ...
        if lr_history:
            ...
            logger.info(f"Initial LR: {lr_history[0]:.2e}")
            logger.info(f"Final LR: {lr_history[-1]:.2e}")
            logger.info(f"Min LR reached: {min(lr_history):.2e}")
            logger.info(f"Max LR: {max(lr_history):.2e}")
